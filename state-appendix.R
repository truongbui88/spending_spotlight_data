...
# Do the same for non CPI adjusted data
...
total_rev <- ss_data |>
    ...
    mutate(
        `Total` = `Total Revenue`,
        `Per Student` = `Total Revenue - Per Pupil`
    ) |>
    select(Year, State, `Total`, `Per Student`) |>
    group_by(Year, State) |>
    pivot_longer(
        !c(Year, State),
        names_to = "Format",
...
fed_rev <- ss_data |>
    ...
    mutate(
        ...
        `Per Student` = `Federal Revenue - Per Pupil`
    ) |>
    select(Year, State, `Total`, `Per Student`) |>
    group_by(Year, State) |>
    pivot_longer(
...
state_rev <- ss_data |>
    ...
    mutate(
        `Total` = `State Revenue`,
        `Per Student` = `State Revenue - Per Pupil`
    ) |>
    select(Year, State, `Total`, `Per Student`) |>
    group_by(Year, State) |>
    pivot_longer(
...
local_rev <- ss_data |>
    ...
    mutate(
        `Total` = `Local Revenue`,
        `Per Student` = `Local Revenue - Per Pupil`
    ) |>
    select(Year, State, `Total`, `Per Student`) |>
    group_by(Year, State) |>
    pivot_longer(
...
# Combine all revenue data into one data frame
rev_data_noncpi <- left_join(total_rev, fed_rev, by = c("Year", "State", "Format")) |>
    left_join(state_rev, by = c("Year", "State", "Format")) |>
    left_join(local_rev, by = c("Year", "State", "Format"))


# Combine CPI adjusted and non CPI adjusted data
rev_data_total <- rbind(rev_data, rev_data_noncpi)

# Save data
write_csv(rev_data_total, "output_data/appendix/rev_data_total.csv")
