...
# Federal Revenue
federal_filter_and_map <- function(state) {
    state_data <- rev_data %>%
        filter(State == state)
    map2(state_data$Year, state_data$`Federal Revenue`, function(x, y) {
        list(x = unbox(as.character(x)), y = unbox(y))
    })
}

# Apply the function to each state
federal_revenue_indexed <- setNames(lapply(states, federal_filter_and_map), states)

federal_revenue_indexed_json <- toJSON(federal_revenue_indexed, pretty = TRUE)
write(federal_revenue_indexed_json, "output_data/appendix/federal_revenue_indexed.json")

# State Revenue
state_filter_and_map <- function(state) {
    state_data <- rev_data %>%
        filter(State == state)
    map2(state_data$Year, state_data$`State Revenue`, function(x, y) {
        list(x = unbox(as.character(x)), y = unbox(y))
    })
}

# Apply the function to each state
state_revenue_indexed <- setNames(lapply(states, state_filter_and_map), states)

state_revenue_indexed_json <- toJSON(state_revenue_indexed, pretty = TRUE)
write(state_revenue_indexed_json, "output_data/appendix/state_revenue_indexed.json")

# Local Revenue
local_filter_and_map <- function(state) {
    state_data <- rev_data %>%
        filter(State == state)
    map2(state_data$Year, state_data$`Local Revenue`, function(x, y) {
        list(x = unbox(as.character(x)), y = unbox(y))
    })
}

# Apply the function to each state
local_revenue_indexed <- setNames(lapply(states, local_filter_and_map), states)

local_revenue_indexed_json <- toJSON(local_revenue_indexed, pretty = TRUE)
write(local_revenue_indexed_json, "output_data/appendix/local_revenue_indexed.json")



# Basic revenue data frame: state, local, and federal
